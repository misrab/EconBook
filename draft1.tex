...
\begin{document}
	...
	\chapter{The Problem}
		...
		\section{Incentives}
			...
			% Intro: not psychology. We're going to look into incentives for people to work in a focused context.
			...
			% What were the 1st and 2nd Industrial Revolution
			% ## By how much exactly did output increase: which countries most, how much per capita in those countries vs in world
			% When did Communism/Socialism arise? What have people argued as its main causes?
			% Where does output per capita stand today, around the world?
			% -------
			% With that background, realities of what incentives we need. The market for labor: (try to bring in some stats)
			%	- how many people love their work, some surveys? how many enjoy...how many bear with it for long term purpose, how many just hate it..... ==> come up with a novel illustration/graph of this
			%	- if so many unfulfilled etc (I suspect), why work? ==> wage distribution ==> how much for needs, how much for wants, and so what are driving factors
			%	- what kind of jobs do people take up: SECTOR composition, INDUSTRY composition, TYPE composition (entrepreneur, sme, big biz) ==> bargaining power
			%		==> do look closely at bargaining power: suppliers and demanders of labor
			% segway to next section. Hint at idea of productive wells and who how the production is distributed. Spend some time emphasizing this as a MAJOR point of book
			
		
		 % The problem of wealth distribution, with a number of illustrated examples
		 % Now getting deep into point of wells of production and where the wealth goes
		 \section{A Rich Man's Game}
		 	% wealth distribution around the world
			% types of jobs and relative wealth: where is the money going
		 
		% The problem of job dissatisfaction, with...
		\section{My Job Sucks}
			% Intro: this section is more fuzzy. More like a rally / oration on soul wrenching jobs. Will have to be inspirational. We are humans not machines, so a holistic book will include this part.
		...
		\section{All Work and No Play Makes Jack a Broken Economy}
			% yep worth writing about, especially after last more emotional section
